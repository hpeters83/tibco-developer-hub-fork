...
|---|---|---|---|
| **Standard** (`devhub-bundled-<os>-<arch>.zip`) | The default. Everything works except that rendering a TechDocs page needs Python. | On first time you open a docs page, the launcher builds a small Python environment for `mkdocs` — this needs **Python 3 already installed** and **one-time internet access**. | Smaller |
...
```bash
# macOS / Linux
curl -fsSL https://raw.githubusercontent.com/hpeters83/tibco-developer-hub-fork/dev_hub_portable/DevHub_Portable/install.sh | DEVHUB_TECHDOCS=1 bash
```
...
```bash
# macOS / Linux
curl -fsSL https://raw.githubusercontent.com/hpeters83/tibco-developer-hub-fork/dev_hub_portable/DevHub_Portable/install.sh | bash -s -- --port 8088
```
