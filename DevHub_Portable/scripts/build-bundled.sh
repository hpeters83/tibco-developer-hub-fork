...
#
# Unlike build-portable.sh (which ships the full ~90k-file node_modules), this
# variant uses esbuild to collapse the backend into a single index.js and ships
# only a minimal sidecar node_modules (native modules + a few packages that load
# on-disk assets). Result: ~3k files instead of ~96k, so extraction is far faster.
#
...
This is the bundled build: the backend is a single index.js and node_modules/
contains only the native modules and on-disk assets it loads at runtime, so the
folder has far fewer files and extracts much faster than the classic bundle.
