...
.DESCRIPTION
  Like build-portable.ps1, but uses esbuild to collapse the backend into a single
  index.js and ships only a minimal sidecar node_modules (native modules + the few
  packages that load on-disk assets). Result: ~3k files instead of ~96k, so the zip
  extracts far faster.
